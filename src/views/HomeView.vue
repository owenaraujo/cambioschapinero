<template>
  ...
  >
    <!--Nav-->
    <nav id="header" class="fixed w-full z-30 top-0 text-gray-800 bg-white">
      <div
      ...
      >
        ...
        >
          <p class="uppercase tracking-loose w-full">
            ¿Necesitas enviar dinero a Venezuela?
          </p>
          <h1 class="my-4 text-5xl font-bold leading-tight">
            Somos la opción ideal!
          </h1>
    ...
    <section class="bg-white border-b py-8 text-gray-800" id="cotizar">
      <div class="container mx-auto flex flex-wrap pt-4 pb-12">
        <h2 class="w-full my-2 text-5xl font-bold leading-tight text-center">
          cotiza tu envio
        </h2>
        <div class="grid mx-auto gap-6 mb-6 md:grid-cols-3">
          <VueNumberFormat
          ...
          <VueNumberFormat
            class="bg-gray-50 border border-gray-300 text-gray-900 text-lg rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
            v-model:value="dolar"
            @keyup="calculate(`dolar`)"
            :options="{ prefix: 'USD ' }"
          ></VueNumberFormat>
        </div>
      </div>
    </section>
    <section class="bg-gray-100 py-8" style="display: none">
    ...
    <section class="container mx-auto text-center py-6 mb-12" id="contacto">
      <h2
      ...
      <h3 class="my-4 text-3xl leading-tight">
        envianos un mensaje a nuestro whatsapp
      </h3>
      <button
        @click="irUrl"
        class="mx-auto lg:mx-0 hover:underline bg-white text-gray-800 font-bold rounded-full my-6 py-4 px-8 shadow-lg focus:outline-none focus:shadow-outline transform transition hover:scale-105 duration-300 ease-in-out"
...
export default {
  setup() {
    ...
    let tasaBss = ref(320);
    let tasaDolar = ref(17.12);
    function calculate(valor) {
      if (valor === "pesos") {
        let bolivar = parseFloat(pesos.value / tasaBss.value).toFixed(2);
        bss.value = bolivar;
        dolar.value = parseFloat(bss.value / tasaDolar.value).toFixed(2);
      }
      if (valor === "bss") {
        dolar.value = parseFloat(bss.value / tasaDolar.value).toFixed(2);
        pesos.value = parseFloat(bss.value * tasaBss.value).toFixed(2);
      }
      if (valor === "dolar") {
        bss.value = parseFloat(tasaDolar.value * dolar.value).toFixed(2);
        pesos.value = parseFloat(bss.value * tasaBss.value).toFixed(2);
      }
    }
    let pesos = ref();
